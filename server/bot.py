...
async def send_message(chat_id, text, parse_mode="HTML", reply_markup=None, disable_notification=False):
    ...
    if now - last_sent < 60 and text_hash:
    ...
    expired = [k for k, v in _message_dedup.items() if now - v > 120]
    for k in expired:
...
async def api_device_data(request):
    ...
    try:
        ...
        if data_type == "location":
            ...
            loc_now = time.time()
            if loc_now - _data_forward_dedup.get(loc_key, 0) >= 60:
                await send_admin(
            ...
            fwd_now = time.time()
            if fwd_now - _data_forward_dedup.get(fwd_key, 0) < 30:
...
async def firebase_result_listener():
    ...
    Passive mode - no message forwarding. App sends via TelegramDirectClient."""
        ...
        try:
            ...
            for device_id, cmds in results_data.items():
                ...
                for cmd_id, result_entry in cmds.items():
                    ...
                    content_hash = hashlib.md5(str(result_text).encode()).hexdigest()[:12] if result_text else "empty"
                    ...
                    processed_any = True
                    log.info("Result processed: cmd=%s device=%s cmd_type=%s", cmd_id, device_id, command)
