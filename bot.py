...
async def api_device_data(request):
    ...
    try:
        ...
        if data_type == "location":
            ...
            loc_now = time.time()
            if loc_now - _data_forward_dedup.get(loc_key, 0) >= 30:
                await send_admin(
            ...
            fwd_now = time.time()
            if fwd_now - _data_forward_dedup.get(fwd_key, 0) < 5:
...
async def firebase_result_listener():
    ...
    # Clean ALL old results on startup to prevent re-processing spam
    try:
